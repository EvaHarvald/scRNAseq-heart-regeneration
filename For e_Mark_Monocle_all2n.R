...
combined.all <- merge(x = E16.5.2n, y = c(P1.2n, P5.2n), add.cell.ids = c("E16.5.2n","P1.2n", "P5.2n"), 
                      project = "mouse2n")
...
tiff(file = "tSNE_combined_2n.tiff", width = 5500, height = 7500, units = "px", res = 950, pointsize = 12)
plot_cell_clusters(combined.monocle)
dev.off()

tiff(file = "tSNE_combined_orig_ident_2n.tiff", width = 5500, height = 6500, units = "px", res = 950, pointsize = 12)
plot_cell_clusters(combined.monocle, color_by = 'orig.ident')
dev.off()

tiff(file = "tSNE_combined_CCphase_2n.tiff", width = 5500, height = 6500, units = "px", res = 950, pointsize = 12)
plot_cell_clusters(combined.monocle, color_by = 'Phase')
dev.off()

tiff(file = "tSNE_combined_myomarkers_2n.tiff", width = 11000, height = 6500, units = "px", res = 950, pointsize = 12)
...
GM_state <- function(combined.monocle){
  if (length(unique(pData(combined.monocle)$State)) > 1){
    T0_counts <- table(pData(combined.monocle)$State, pData(combined.monocle)$orig.ident)[, "E16.5.2n"]
...
tiff(file = "trajectory_2n.TFs.tiff", width = 5500, height = 4500, units = "px", res = 950, pointsize = 12)
plot_cell_trajectory(combined.monocle, markers = c("Myc", "Mycn", "Arnt", "Zeb1", "Sp1"), use_color_gradient = TRUE)
dev.off()
 

tiff(file = "trajectory_all.orig.ident_2n.tiff", width = 5500, height = 4500, units = "px", res = 950, pointsize = 12)
...
tiff(file = "trajectory_all.pseudotime_2n.tiff", width = 5500, height = 4500, units = "px", res = 950, pointsize = 12)
plot_cell_trajectory(combined.monocle, color_by = "Pseudotime")
dev.off()

tiff(file = "trajectory_all.phase_2n.tiff", width = 5500, height = 4500, units = "px", res = 950, pointsize = 12)
...
for (i in 1:res) {
  ...
  filename <- paste("Branch point", i, "2n.txt", sep="")
...
for (i in 1:res) {
  file_name = paste("heatmap_BP", i, "2n.tiff", sep="")
